...
What a good answer looks like once the loop asks for one. `SKILL.md` holds the
loop — the ladder, the renders, when to stop; this file holds the judgment the
ladder calls on. Read it when the architecture loop opens, before cutting
symbols, and again whenever a rung asks what shape a thing should take.

## Orthodox

Orthodox C++ spirit generalized: **data and machine constraints first, language fashion second.** Where this collides with "modern defaults", this section wins unless the task explicitly overrides it.
...
- **Fast feedback**: keep build/test/edit loops short; a multi-second routine loop is a defect worth fixing when this code is touched.
...
- Hidden allocation or type erasure on latency-sensitive paths; **lifetime and allocation policy are part of the API**.
...
- Settle inputs/outputs, frequency, size, lifetime, and failure modes before code — that is the `PROBLEM` block in `SKILL.md`, and `PLAN.md` is where it lands.
...
- Prefer **small tools** and stable **text streams** over bespoke IPC when the fit is good.
- **stdin → stdout**, nonzero exit on failure, minimal flags.
...
- **Stable names.** A breaking change is announced with a migration path and a removal date; no silent behavior drift.
